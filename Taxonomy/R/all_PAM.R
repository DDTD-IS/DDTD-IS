#' @title Silhouette recommendation for partitioning around medoids (PAM) clustering algorithm
#'
...
#' any potential number of clusters (specified by No_Clusters) for the clustering algorithm \link[cluster]{pam}. The result is a recommendation, indicating
...
#'   Distance matrix of class \code{dist} that is used as input for the PAM clustering algorithms
...
all_PAM <-
  function(No_Clusters,  dist) {
    ...
    for (i in 2:No_Clusters) {
      Clusterings_all = cbind(Clusterings_all,
                              clustering_pam(distance_matrix = dist, number_clusters = i))
    ...
    if (Silhouettes_all[max_index] > max_sil) {
      ...
      sil_recommendation = c("pam", max_index + 1, max(Silhouettes_all))
    }
